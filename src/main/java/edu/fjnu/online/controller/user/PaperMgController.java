...
public class PaperMgController {
	...
	public String qryPaperDetail(String paperId,String userId,Model model, HttpSession session){
		...
		List<Question> inpList = new ArrayList<Question>();
		List<Question> desList = new ArrayList<Question>();
		System.out.println("enter paperexam");
		for(int i = 0;i<ids.length;i++){
            System.out.println(ids[i]);
			question = questionService.getQuestion(Integer.parseInt(ids[i]));
            System.out.println(question.getQuestionType());
            System.out.println(question.getQuesName());
            selList.add(question);
			/*if("0".equals(question.getQuestionType())){//单选
				selList.add(question);
                System.out.println(question.getQuesName());
			}
			if("4".equals(question.getQuestionType())){//填空
				inpList.add(question);
			} 
			if("5".equals(question.getQuestionType())){//简答题
				desList.add(question);
			}*/
		}
	...
	public String toMyPaperPage(User user,Model model, HttpSession session){
		...
		user = userService.getStu(user);
		Map map =new HashMap();
		map.put("gradeId", user.getGrade());
		String gradeId = user.getGrade();
		System.out.printf("gradeId is %s",gradeId);
		//List<Paper> paper = paperService.getUserPaperById(user.getUserId());
		List<Paper> paper1 = paperService.getUndoPaper(map);
		Course course = null;
        System.out.println("before enter pagerquery");
		for(Paper p : paper1){
		    System.out.println("enter pagerquery");
			course = courseService.get(Integer.parseInt(p.getCourseId()));
		...
		for(Paper p : paper){
			course = courseService.get(Integer.parseInt(p.getCourseId()));
			p.setCourseId(course.getCourseName());
            System.out.println("enter querypager");
		}
		model.addAttribute("user", user);
		model.addAttribute("paper", paper);
